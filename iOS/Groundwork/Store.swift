...
final class Store: ObservableObject {
    ...
    private let fileName = "groundwork_entries.json"
    ...
    private func seed() {
        entries = [
            Entry(date: Calendar.current.date(byAdding: .day, value: -1, to: Date())!, value: 7.0),\nEntry(date: Calendar.current.date(byAdding: .day, value: -2, to: Date())!, value: 8.0),\nEntry(date: Calendar.current.date(byAdding: .day, value: -3, to: Date())!, value: 6.0)
        ]
    }
    private func load() {
